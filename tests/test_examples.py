...
def test_create_offer_nested(client: TestClient):
    ...
    resp = client.post("/api/v1/demo/examples/models/offers", json=payload)
...
def test_create_multiple_images(client: TestClient):
    """请求体为模型列表。"""
    resp = client.post(
        "/api/v1/demo/examples/models/images",
        json=[
...
def test_create_index_weights(client: TestClient):
    """请求体为 dict[int, float]。"""
    resp = client.post("/api/v1/demo/examples/models/index-weights", json={"1": 0.5, "2": 0.5})
    assert resp.status_code == 200
    body = resp.json()
    assert body["1"] == 0.5
...
def test_create_index_weights_invalid_value_type(client: TestClient):
    """值非 float 应 422。"""
    resp = client.post("/api/v1/demo/examples/models/index-weights", json={"1": "not-a-number"})
...
def test_path_validation(client: TestClient):
    ...
    resp = client.get("/api/v1/demo/examples/params/path/3", params={"size": 5.0})
    assert resp.status_code == 200
    assert resp.json() == {"item_id": 3, "size": 5.0}


def test_path_validation_id_too_small(client: TestClient):
    assert client.get("/api/v1/demo/examples/params/path/0").status_code == 422


def test_path_validation_size_out_of_range(client: TestClient):
    assert client.get("/api/v1/demo/examples/params/path/1", params={"size": 11}).status_code == 422


def test_multi_query(client: TestClient):
    """同名查询参数收集为列表。"""
    resp = client.get("/api/v1/demo/examples/params/multi-query", params=[("q", "a"), ("q", "b")])
    assert resp.status_code == 200
...
def test_validated_query_fixedquery(client: TestClient):
    """正则 ^fixedquery$ 校验通过。"""
    resp = client.get("/api/v1/demo/examples/params/validated-query", params={"item-query": "fixedquery"})
    assert resp.status_code == 200
...
def test_validated_query_pattern_mismatch(client: TestClient):
    """不匹配 pattern 应 422。"""
    resp = client.get("/api/v1/demo/examples/params/validated-query", params={"item-query": "nope"})
    assert resp.status_code == 422


def test_optional_query_with_q(client: TestClient):
    resp = client.get("/api/v1/demo/examples/params/optional-query/abc", params={"q": "hi"})
    assert resp.status_code == 200
    assert resp.json() == {"item_id": "abc", "q": "hi"}


def test_optional_query_without_q(client: TestClient):
    assert client.get("/api/v1/demo/examples/params/optional-query/abc").json() == {"item_id": "abc"}


def test_bool_query_short_true(client: TestClient):
    """short=true 返回精简响应。"""
    resp = client.get("/api/v1/demo/examples/params/bool-query/x", params={"short": "true"})
    assert resp.status_code == 200
    assert resp.json() == {"item_id": "x"}


def test_bool_query_short_false(client: TestClient):
    resp = client.get("/api/v1/demo/examples/params/bool-query/x", params={"short": "false"})
    assert resp.json() == {"item_id": "x", "description": "这是一段很长的描述"}


def test_required_query_missing(client: TestClient):
    """needy 必选查询参数缺失应 422。"""
    assert client.get("/api/v1/demo/examples/params/required-query/x").status_code == 422


def test_required_query_provided(client: TestClient):
    resp = client.get("/api/v1/demo/examples/params/required-query/x", params={"needy": "v"})
    assert resp.status_code == 200
...
def test_read_user_agent(client: TestClient):
    resp = client.get("/api/v1/demo/examples/headers/user-agent", headers={"User-Agent": "pytest"})
...
def test_read_x_token_multi(client: TestClient):
    ...
    resp = client.get(
        "/api/v1/demo/examples/headers/x-token",
...
def test_read_x_uid_multi(client: TestClient):
    resp = client.get(
        "/api/v1/demo/examples/headers/x-uid",
...
def test_read_session_cookie(client: TestClient):
    resp = client.get("/api/v1/demo/examples/cookies/session", cookies={"session_token": "abc"})
    assert resp.json() == {"session_token": "abc"}


def test_read_headers_and_cookies(client: TestClient):
    resp = client.get(
        "/api/v1/demo/examples/headers-cookies",
...
def test_redirect(client: TestClient):
    """重定向到 /api/v1/demo/items/。"""
    resp = client.get("/api/v1/demo/examples/responses/redirect", follow_redirects=False)
    assert resp.status_code in (301, 302, 303, 307, 308)
    assert resp.headers["location"].endswith("/api/v1/demo/items/")


def test_unicorn_normal(client: TestClient):
    resp = client.get("/api/v1/demo/examples/responses/unicorns/sweetie")
    assert resp.status_code == 200
...
def test_unicorn_exception(client: TestClient):
    ...
    resp = client.get("/api/v1/demo/examples/responses/unicorns/yolo")
...
def test_http_exception_with_dict_detail(client: TestClient):
    ...
    resp = client.get("/api/v1/demo/examples/responses/http-exception/invalid")
...
def test_header_error_with_custom_header(client: TestClient):
    ...
    resp = client.get("/api/v1/demo/examples/responses/header-error/invalid")
    assert resp.status_code == 404
    assert resp.headers.get("x-error") == "There goes my error"


def test_header_error_normal(client: TestClient):
    resp = client.get("/api/v1/demo/examples/responses/header-error/ok")
    assert resp.status_code == 200
    assert resp.json() == {"item": "ok"}


def test_custom_json_response(client: TestClient):
    resp = client.get("/api/v1/demo/examples/responses/custom/99")
    assert resp.status_code == 200
...
def test_dependency_common_items(client: TestClient):
    resp = client.get(
        "/api/v1/demo/examples/dependencies/common/items",
...
def test_dependency_common_users(client: TestClient):
    resp = client.get("/api/v1/demo/examples/dependencies/common/users")
    assert resp.status_code == 200
...
def test_dependency_class(client: TestClient):
    resp = client.get(
        "/api/v1/demo/examples/dependencies/class",
...
def test_dependency_sub_query_admin(client: TestClient):
    ...
    resp = client.get("/api/v1/demo/examples/dependencies/sub", params={"q": "admin"})
    assert resp.status_code == 200
    assert resp.json() == {"q": "admin (checked)"}


def test_dependency_sub_query_other(client: TestClient):
    resp = client.get("/api/v1/demo/examples/dependencies/sub", params={"q": "user"})
...
def test_dependency_api_key_protected_ok(client: TestClient):
    """携带正确 X-API-Key 通过。"""
    resp = client.get(
        "/api/v1/demo/examples/dependencies/api-key-protected",
...
def test_dependency_api_key_protected_missing(client: TestClient):
    ...
    assert client.get("/api/v1/demo/examples/dependencies/api-key-protected").status_code == 422
...
def test_dependency_route_protected(client: TestClient):
    resp = client.get(
        "/api/v1/demo/examples/dependencies/route-protected",
        headers={"X-API-Key": "secret-key"},
    )
    assert resp.status_code == 200
...
def test_dependency_yield_db(client: TestClient):
    """yield 依赖提供数据库句柄。"""
    resp = client.get("/api/v1/demo/examples/dependencies/yield-db")
    assert resp.status_code == 200
