...
def test_list_items_pagination(client: TestClient):
    """skip/limit 正确切片。"""
    assert len(client.get("/api/v1/items/?skip=1").json()) == 2
    assert len(client.get("/api/v1/items/?skip=1&limit=1").json()) == 1
...
def test_read_item_basic(client: TestClient):
    """获取商品详情，带可选查询参数 q。"""
    resp = client.get("/api/v1/items/1", params={"q": "hello"})
    assert resp.status_code == 200
    assert resp.json() == {"item_id": 1, "q": "hello"}


def test_read_item_without_q(client: TestClient):
    assert client.get("/api/v1/items/5").json() == {"item_id": 5, "q": None}
...
def test_read_item_43_returns_custom_headers(client: TestClient):
    ...
    resp = client.get("/api/v1/items/43")
    assert resp.status_code == 200
...
def test_read_item_id_must_be_int(client: TestClient):
    """非整数 item_id 应 422。"""
    assert client.get("/api/v1/items/abc").status_code == 422
...
def test_create_item_validation_failure(client: TestClient):
    ...
    assert client.post("/api/v1/items/", json={"name": "", "price": 1}).status_code == 422
    assert client.post("/api/v1/items/", json={"name": "x", "price": 0}).status_code == 422
    assert client.post("/api/v1/items/", json={"name": "x"}).status_code == 422  # 缺 price


def test_update_item(client: TestClient):
    """PUT 更新商品，回显新名称。"""
    resp = client.put("/api/v1/items/7", json={"name": "Bar", "price": 9.9})
    assert resp.status_code == 200
...
def test_read_item_filtered_unset_excluded(client: TestClient):
    ...
    body = client.get("/api/v1/items/filtered/foo").json()
