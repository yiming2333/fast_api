...
def test_form_create_item_optional_description(client: TestClient):
    resp = client.post("/api/v1/forms/items", data={"name": "Foo", "price": "1"})
    assert resp.status_code == 200
...
def test_form_create_item_price_must_be_positive(client: TestClient):
    assert client.post(
        "/api/v1/forms/items", data={"name": "x", "price": "0"}
    ).status_code == 422


def test_form_create_item_missing_required(client: TestClient):
    """缺 name 或 price 应 422。"""
    assert client.post("/api/v1/forms/items", data={"name": "x"}).status_code == 422
    assert client.post("/api/v1/forms/items", data={"price": "1"}).status_code == 422


def test_form_with_file(client: TestClient):
    """表单字段与上传文件混合提交。"""
    resp = client.post(
        "/api/v1/forms/items-with-file",
